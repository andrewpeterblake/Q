...
plot(pmh)
```
or just the first few to better see the algoritmn in action:
```{r warning=FALSE}
#| warning: false
#| echo: false
...
Here's a line-by-line example in Matlab^&trade;^ of two parameters distributed multivariate normal. We can use the Matlab command `mvnpdf(b,mu,V)` to evaluate the density of any vector $b$ distributed $N(mu,V)$. Similar examples can be found in, e.g., @BDA.

The underlying densities are assumed to be:

<center>
![](MH1.png){width=65%}
</center>

We will use the very simple Matlab code:
```{matlab}
b    = [-3;-3];
V    = 3*eye(2); V(2,2) = 0.4;
mu   = [-0.2; 2.5];
...
naccept   = 0;
for i = 1:n-1
    bnew = mvnrnd(bold,s^2*eye(2))';
    rold = mvnpdf(bold,mu,V);
    rnew = mvnpdf(bnew,mu,V);
    if rnew/rold > rand(1,1)
        bold = bnew;
        naccept = naccept+1;
    end
    allb(:,i+1) = bold;
    subplot(1,2,1); plot(allb(1,i:i+1),allb(2,i:i+1),'r:*'); hold on
    if i > burn+1
      subplot(1,2,2); plot(allb(:,burn+1:end)'); xlim([1 1024]); axis square; title('MH sequence')
    end
end
...
The draws in two dimensions are plotted left -- they go red once we've passed the burn in period. The draws past the burn in are plotted right.

{{< video https://vimeo.com/864729727 >}}

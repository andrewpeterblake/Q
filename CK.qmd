...
Th Poisson distribution is a nice one to consider as the maximum likelihood estimate can be calculated easily -- essentially in your head.[^For example @Green1997 constructs a Poisson distribution example where the chosen observations yield an exact estimate of the underlying parameter of the distribution.]
...
ggplot(t) + 
  geom_histogram(aes(x=ests), 
                 fill = "blue", alpha=.33, color=NA, bins=20)
...
df     <- tibble(theta = theta) %>%
  mutate(
    `lambda == 1` = (exp(-1)*1^(theta))/(factorial(theta)),
...
tt <- bind_cols(t2$r, .name_repair = ~ nms) %>%
  mutate(lambda = l)
...
tt %>%
  ...
  geom_line(aes(x=lambda, y=value, color=name),
            show.legend = FALSE) +
  geom_vline(aes(xintercept=lam), color="red", linetype=3, linewidth=0.75) +
...
$$ as the \`innovation' in predicted $\beta_{t+1|t}$ where we have some realized $\beta_{t+1}$ drawn from its probability distribution. Carter-Kohn smoother comprises updates to the conditional expectations that use this news.
$$
\begin{align}

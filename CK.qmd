...
A suitable numerical maximization routine will (in principle) maximize the likelihood straightforwardly. Often use Chris Sims' **csminwel** in Matlab as well-suited to this type of problem. Can show (via Cramer-Rao) that
$$
...
- This is a _fixed interval smoother_; often used for full sample estimates of $\beta_t$

- Remember we already have an estimate of $\beta_{T|T}$ from the Kalman filter so _smoothers work backwards_; we sketch a derivation here

...
Alternatives to what follows can be found in @KimNelson, @Harvey89, @Hamilton1994, or @Durbin. There are many books devoted to the Kalman filter [as a causal Amazon search](https://www.amazon.co.uk/s?k=kalman+filtering&i=stripbooks) mostly from an engineering perspective. However econometricians since @HarveyPierse have used it in a way somewhat different from standard engineering applications. We will cover the filter and then look at a simple example. 
...
**Key points**: 
 ...
 - Many quantities routinely used to build models and analyse policy are _unobservable_.
...
Imagine we had a time-varying parameter model where we estimate $\beta_t$; this becomes a time series so we have a lot of parameters to estimate: we outline an estimation method here, which we will then characterise as the _Kalman Filter_.
...
Filtering is specifically this: perhaps we have some estimates already of $\beta_t$ for $t=1...t-1$, then given the new period-$t$ observation of $y_t$ how should we estimate a new value of $\beta_t$? This immediately implies a recursive structure to estimation problems, consistent with on-line (or real-time) estimation.
The Kalman filter uses the period $t$ news available from observed $y_t$ to update our estimates of $\beta_t$ using the regression lemma.
...
$$
We can use to make the _conditional_ forecast
$$
...
We can turn this around, as it must be the _prediction errors_ are given by
$$
\begin{aligned}
...
Now we find $E [\beta_t | \psi_t]$ -- the best prediction of the unknown coefficient vector given _current_ information. Using the regression lemma we know that
$$
\begin{align}
...
- Stationary models can use steady-sate
- Non-stationary models use something which is often (confusingly) called a _diffuse prior_ (zero mean, large variance)
...
\end{eqnarray*}
$$
where $\Phi =\frac{Tm}{2}\log \left( 2\pi \right)$ and $\theta$ are all the non-state parameters to be estimated.

We can use the Kalman filter to obtain $\eta_{t|t-1}$ and $f_{t|t-1}$ as they are the _prediction error_ and its _variance_. This is the ***prediction error decomposition*** of the log-likelihood. 

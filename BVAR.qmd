...
We will the Fed Funds rate, annual GDP growth and annual CPI inflation data from FRED, retrieved `r Sys.Date()`. These are:
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
############################
# Data
############################
...
First we use quarterly US Growth (FRED series [A191RO1Q156NBEA](https://fred.stlouisfed.org/series/A191RO1Q156NBEA)) and CPI (FRED series [CPALTT01USQ661S](https://fred.stlouisfed.org/series/CPALTT01USQ661S)) expressed as the annual inflation rate from `r head(Y$Date,1)` to `r tail(Y$Date,1)` in a bi-variate BVAR. The last ten observations are:
```{r echo=FALSE}
...
Now we add the FedFunds rate (FRED series [FEDFUNDS](https://fred.stlouisfed.org/series/FEDFUNDS)), so the last ten periods of the data set is now:
```{r echo=FALSE}
Y   <- Y0
knitr::kable(tail(Y, 10))
```
